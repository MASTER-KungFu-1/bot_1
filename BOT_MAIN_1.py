...
def parser_find():
    ...
    req = requests.get(url,headers=headers)
    src = req.text
    with open("Bot/dict_2.json","w") as file:
            json.dump(src,file,indent=4,ensure_ascii=True)    

    with open("Bot/dict_2.json", "r") as read_file:
        data = json.load(read_file)
    data:dict = json.loads(data)
    data = data.get("data")
    all_find = data
...
def parser_prepod_info(prepod:int,date):
    ...
    try:
        ...
        req = requests.get(url,headers=headers)
        src = req.text
    with open("Bot/dict_3.json","w") as file:
            json.dump(src,file,indent=4,ensure_ascii=False)    

    with open("Bot/dict_3.json", "r") as read_file:
        data = json.load(read_file)
...
#     elif info == "16:00":
#         return 5
# def time_check(hours,minutes, end_hours, end_minutes):
#     if hours == end_hours and minutes == end_minutes -6: return True
#     else: return False
# async def send_message_rasp(dp: Dispatcher, groupp:int):
#     main  = parser(groupp,datetime.datetime.now())
#     time = datetime.datetime.now()
#     for i in main[time.weekday()]:
#         if i.get("начало") == "08:30" :
#             print(i)
    
    # await dp.bot.send_message(f"Скоро начнётся пара {number_lesson(main[])}")
...
# async def para_1(message: types.Message,groupp):
#     time = datetime.datetime.now()
#     a = []
#     main = parser(groupp,time.weekday())
#     for i in main[time.weekday()]:
#         if i.get("начало") == "08:30":
#             a.append(i.get("дисциплина"))
#             a.append(i.get("преподаватель"))
#             a.append(i.get("начало"))
#             a.append(i.get("конец"))
#             a.append(i.get("аудитория"))

#     await message.answer(f"Скоро начнётся 1 пара: {a[0]}\nПреподаватель: {a[1]}\nНачало в: {a[2]} До: {a[3]}\nАудитория: {a[4]}")

@dp.message_handler(commands=["start","now","change","help","tomorrow",'date',"notification","find"],state=None)
async def cmd_start(message: types.Message):
    ...
    if message.text.lower() == "/notification":
        if id_notification == 0:
            with engine.cursor() as cursor:
                sql = f"UPDATE `rasp_db`.`users` SET `notifications` = '1' WHERE (`id` = '{message.from_user.id}');"
                cursor.execute(sql)
                engine.commit()
            await message.reply("Вы включили уведомления о парах!")
        else:
            with engine.cursor() as cursor:
                sql = f"UPDATE `rasp_db`.`users` SET `notifications` = '0' WHERE (`id` = '{message.from_user.id}');"
                cursor.execute(sql)
                engine.commit()
            await message.reply("Вы выключили уведомления о парах!")

    if "/date" in message.text.lower():
        await FSMAdmin.date.set()
        await message.answer("напишите дату в формате 00:00")
            ...
            if all_days[time.weekday()] == []:
                ...
                for i in range(len(all_days[time.weekday()])):
                    ...
                    if info[i][2] == info[i-1][2] and i>0:
                        ...
                        message_send.append(f"Пара № {i+1-f}: {info[i][0]}\nПреподаватель: {info[i][1]}\nНачало в: {info[i][2]} До: {info[i][3]}\nАудитория: {info[i][4]}")                     
                    else:
...
async def change_info(message: types.Message,state:FSMContext):
    ...
    if len(data)==5:
        ...
        await message.answer("Неправильно написана дата, напишите в формате 01:01")
...
async def one_more_find(message: types.Message,state:FSMContext):
    ...
    # for  i in o:
    #     if "Щерба Лидия".lower() in i.get("name").lower():
    #         f = i.get("id")
    #         a = parser_prepod_info(f,datetime.datetime.now())
